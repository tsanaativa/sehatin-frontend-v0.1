...
const ChatBubble = ({ isSent, timestamp, children }: ChatBubbleProps) => {
  return (
    ...
    >
      <div
        className={`py-3 px-4 bg-primary-${isSent ? 'dark' : 'border'} font-semibold text-${!isSent ? 'primary-dark' : 'light'} rounded-2xl rounded-t${isSent ? 'r' : 'l'}-none max-w-[60%] md:max-w-[45%]`}
      >
        {children}
      </div>
